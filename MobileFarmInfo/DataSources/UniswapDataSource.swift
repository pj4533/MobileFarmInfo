...
class UniswapDataSource {
    ...
    func getToken(address: String, withSuccess success: ((_ token: Token?) -> Void)?, failure: ((_ error: Error?) -> Void)? ) {
        let parameters : [String:Any] = [
            "query" : "query($tokenId: String!) {  token(id:$tokenId) { id name derivedETH totalSupply totalLiquidity decimals   }  }",
            "variables" : [
        ...
        let task = URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            if let data = data {
                do {
                    ...
                    let graphQLResponse = try decoder.decode(GraphQLResponse.self, from: data)
                    DispatchQueue.main.async {
                        success?(graphQLResponse.data?.token)
                    }
                } catch let error {
                    DispatchQueue.main.async {
                        failure?(error)
